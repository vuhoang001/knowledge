...
|---|---|---|
| **Grain** | Một dòng của bảng đại diện cho cái gì | [→](../data-modeling/reference/grain.md) |
| **Fact** | Bảng chứa số đo được, dài và hẹp, mọc thêm mỗi ngày | [→](../data-modeling/reference/fact-and-dimension.md) |
| **Dimension** | Bảng mô tả thực thể, ngắn và rộng, đổi chậm | [→](../data-modeling/reference/fact-and-dimension.md) |
| **SCD** | Cách xử lý lịch sử khi thuộc tính dimension thay đổi | [→](../data-modeling/skills/scd.md) |
...
| **Additive** | Số đo cộng được theo mọi chiều | [→](../data-modeling/reference/fact-and-dimension.md) |
| **Semi-additive** | Số đo không cộng được theo thời gian (số dư cuối ngày) | [→](../data-modeling/reference/fact-and-dimension.md) |
| **Late-arriving dimension** | Fact tới trước dimension → chưa tra được surrogate key | [→](../data-modeling/skills/scd.md) |
...
| **Singular test** | File `.sql` trả về **các dòng sai**; trả 0 dòng = pass | [→](../etl/dbt/reference/testing.md) |

## dbt

| Thuật ngữ | Một câu | Chi tiết |
|---|---|---|
| **model** | Một file `.sql` = một `SELECT` → thành view/table | [→](../etl/dbt/reference/models-and-ref.md) |
| **`ref()`** | Cách duy nhất khai báo phụ thuộc — thứ dựng nên DAG | [→](../etl/dbt/reference/models-and-ref.md) |
| **`source()`** | Trỏ tới bảng dbt **không** tạo ra | [→](../etl/dbt/reference/sources-seeds-snapshots.md) |
| **materialization** | dbt bọc gì quanh câu `SELECT` của bạn | [→](../etl/dbt/reference/materializations.md) |
| **incremental** | Chỉ xử lý dòng mới thay vì build lại cả bảng | [→](../etl/dbt/reference/materializations.md) |
| **snapshot** | Công cụ dbt hiện thực SCD Type 2 — **không build lại được** | [→](../etl/dbt/reference/sources-seeds-snapshots.md) |
| **`target/compiled/`** | SQL sau khi render Jinja — thứ warehouse thật sự nhận | [→](../etl/dbt/reference/what-is-dbt.md) |
